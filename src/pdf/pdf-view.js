...
function isLikelyEnglishText(sample) {
	...
	const chineseRatio = chineseChars / stripped.length;

	// If Chinese ratio > 10%, it's not an English document
...
class PDFView {
	constructor(options) {
		...
		// Initial check
		this._preferedColorTheme = darkModeMediaQuery.matches
			? "dark"
			: "light";
		...
		this._history = new History({
			...
			onNavigate: (location) =>
				this.navigate(location, { skipHistory: true }),
		});

		this._overlayPopupDelayer = new PopupDelayer({
			open: !!this._overlayPopup,
		});
		...
		this._pageLabelsPromise = new Promise(
			(resolve) => (this._resolvePageLabelsPromise = resolve)
		);
		...
		let setOptions = () => {
			...
			this._iframeWindow.PDFViewerApplicationOptions.set(
				"isEvalSupported",
				false
			);
			this._iframeWindow.PDFViewerApplicationOptions.set(
		...
		this._iframe.addEventListener("load", () => {
			...
			setTimeout(() => {
				...
				if (options.data.buf) {
					this._iframeWindow.PDFViewerApplication.open({
						data: options.data.buf,
				...
				window.PDFViewerApplication =
					this._iframeWindow.PDFViewerApplication;
				window.if = this._iframeWindow;

				this._iframeWindow.document
					...
					.addEventListener("scroll", (event) => {
						...
						if (this._annotationPopup) {
							...
							if (annotations.length === 1) {
								...
								let rect = this.getClientRectForPopup(
									annotation.position,
									x,
									y
								);
	...
	async _init() {
		...
		this._autoScroll = new AutoScroll({
			container:
				this._iframeWindow.document.getElementById("viewerContainer"),
		});
		...
		this._iframeWindow.PDFViewerApplication.eventBus.on(
			"documentinit",
			this._handleDocumentInit.bind(this)
		);
		...
		this._findController = new PDFFindController({
			...
			onNavigate: async (pageIndex, matchIndex) => {
				let matchPositions =
					await this._findController.getMatchPositionsAsync(
						pageIndex
					);
			...
			onUpdateMatches: ({ matchesCount }) => {
				...
				if (this._pdfjsFindState === FindState.PENDING) {
					...
					(async () => {
						await this._ensureBasicPageData(
							matchesCount.currentPageIndex
						);
						let selectionRanges = getSelectionRanges(
							...
							{
								pageIndex: matchesCount.currentPageIndex,
								offset: matchesCount.currentOffsetEnd + 1,
							}
						);
						result.annotation =
							this._getAnnotationFromSelectionRanges(
								selectionRanges,
								"highlight"
							);
						...
						result.currentSnippet =
							result.snippets[matchesCount.current - 1];
					})();
				}
			...
			onUpdateState: ({ matchesCount, state, rawQuery }) => {
				...
				if (
					this._pdfjsFindState === FindState.PENDING ||
					!rawQuery.length
				) {
					result = null;
				} else if (matchesCount.current) {
	...
	async _init2() {
		...
		this._iframeWindow.PDFViewerApplication.eventBus.on(
			"updateviewarea",
			this._handleViewAreaUpdate.bind(this)
		);
	...
	async _handleDocumentInit() {
		...
		// Default state
			...
			this._iframeWindow.PDFViewerApplication.pdfViewer.currentScaleValue =
				"page-width";
		}
		...
		this._findController.setDocument(
			this._iframeWindow.PDFViewerApplication.pdfDocument
		);
	}

	async _setState(state, skipScroll) {
		if (Number.isInteger(state.scrollMode)) {
			this._iframeWindow.PDFViewerApplication.pdfViewer.scrollMode =
				state.scrollMode;
		}

		if (Number.isInteger(state.spreadMode)) {
			this._iframeWindow.PDFViewerApplication.pdfViewer.spreadMode =
				state.spreadMode;
		}
	...
	async _updateColorScheme() {
		...
		if (this._iframeWindow) {
			this._iframeWindow.document.documentElement.dataset.colorScheme =
				this._colorScheme;
			...
			if (this._colorScheme === "light" && this._lightTheme) {
				...
				root.style.setProperty(
					"--background-color",
					this._lightTheme.background
				);
				this._themeColorScheme = getModeBasedOnColors(
					this._lightTheme.background,
					this._lightTheme.foreground
				);
			} else if (this._colorScheme === "dark" && this._darkTheme) {
				this._iframeWindow.theme = this._darkTheme;
				root.style.setProperty(
					"--background-color",
					this._darkTheme.background
				);
				this._themeColorScheme = getModeBasedOnColors(
					this._darkTheme.background,
					this._darkTheme.foreground
				);
			} else {
	...
	async _initProcessedData() {
		let pageLabels =
			await this._iframeWindow.PDFViewerApplication.pdfDocument.getPageLabels2();
		this._onSetPageLabels(pageLabels);
		this._resolvePageLabelsPromise();
		this._render();
		this._updateViewStats();
		let { pages } =
			await this._iframeWindow.PDFViewerApplication.pdfDocument.getProcessedData();
	...
	async _detectDocumentLanguage() {
		...
		try {
			...
			for (
				let i = 0;
				i < pagesToInspect &&
				sampleText.length < MAX_LANGUAGE_SAMPLE_LENGTH;
				i++
			) {
				...
				if (charsText) {
					...
					if (sampleText.length > MAX_LANGUAGE_SAMPLE_LENGTH) {
						sampleText = sampleText.slice(
							0,
							MAX_LANGUAGE_SAMPLE_LENGTH
						);
					}
				}
			}
			sampleText = sampleText.trim();
			let isEnglish = sampleText
				? isLikelyEnglishText(sampleText)
				: false;
	...
	async _ensureBasicPageData(pageIndex) {
		if (!this._pdfPages[pageIndex]) {
			let pageData =
				await this._iframeWindow.PDFViewerApplication.pdfDocument.getPageData(
					{ pageIndex }
				);
			if (!this._pdfPages[pageIndex]) {
	...
	_detachPage(originalPage, replacing) {
		let pageIndex = originalPage.id - 1;
		this._pages = this._pages.filter(
			(x) => x.originalPage !== originalPage
		);
		if (!replacing) {
	...
	_focusNext(side) {
		let visiblePages =
			this._iframeWindow.PDFViewerApplication.pdfViewer._getVisiblePages();
		let visibleObjects = [];

		let scrollY =
			this._iframeWindow.PDFViewerApplication.pdfViewer.scroll.lastY;
		let scrollX =
			this._iframeWindow.PDFViewerApplication.pdfViewer.scroll.lastX;
		for (let view of visiblePages.views) {
			...
			if (pdfPage) {
				overlays = pdfPage.overlays.filter(
					(x) => x.type !== "reference"
				);
			}
			...
			for (let annotation of this._annotations) {
				if (
					...
					(annotation.position.nextPageRects &&
						annotation.position.pageIndex + 1 === pageIndex)
				) {
		...
		if (this._focusedObject) {
			for (let visibleObject of visibleObjects) {
				if (
					visibleObject.object === this._focusedObject.object &&
		...
		if (focusedObject && side) {
			let otherObjects = visibleObjects.filter(
				(x) => x !== focusedObject
			);
			nextObject = getClosestObject(
				focusedObject.rect,
				otherObjects,
				side
			);
	...
	async renderPageAnnotationsOnCanvas(canvas, viewport, pageIndex) {
		...
		if (
			this._annotations.find(
				(x) =>
					x.position.pageIndex === pageIndex && x.type === "underline"
			)
		) {
	...
	navigateToPosition(position, options = {}) {
		...
		let top =
			y -
			(options.block === "start" ? 0 : clientHeight / 2) -
			verticalPadding;
		...
		let close =
			...
			position.pageIndex <= endPageIndex;
	...
	setPageLabels(pageLabels) {
		...
		for (
			let i = 0;
			i < this._iframeWindow.PDFViewerApplication.pdfViewer._pages.length;
			i++
		) {
			let page =
				this._iframeWindow.PDFViewerApplication.pdfViewer._pages[i];
			let label = pageLabels[i];
			if (label != i + 1) {
				page.div.setAttribute(
					"aria-label",
					`Page: ${label}. Index: ${i + 1}`
				);
			} else {
	...
	setTool(tool) {
		...
		if (tool.type === "hand") {
			this._iframeWindow.PDFViewerApplication.pdfCursorTools.switchTool(
				1
			);
		} else {
			this._iframeWindow.PDFViewerApplication.pdfCursorTools.switchTool(
		...
		this._iframeWindow.document.getElementById(
			"viewerContainer"
		).style.touchAction = tool.type !== "pointer" ? "none" : "auto";
		this._tool = tool;
	}
	...
	setAnnotations(annotations) {
		...
		let affected = getAffectedAnnotations(
			this._annotations,
			annotations,
			true
		);
		...
		if (this._focusedObject?.type === "annotation") {
			if (updated.find((x) => x.id === this._focusedObject.object.id)) {
				this._focusedObject.object = updated.find(
					(x) => x.id === this._focusedObject.object.id
				);
			} else if (
				deleted.find((x) => x.id === this._focusedObject.object.id)
			) {
				this._focusedObject = null;
			}
		}
		...
		if (this._lastFocusedObject?.type === "annotation") {
			if (
				updated.find((x) => x.id === this._lastFocusedObject.object.id)
			) {
				this._lastFocusedObject.object = updated.find(
					(x) => x.id === this._lastFocusedObject.object.id
				);
			} else if (
				deleted.find((x) => x.id === this._lastFocusedObject.object.id)
			) {
				this._lastFocusedObject = null;
			}
		}
	...
	setFindState(state) {
		...
		if (state.active) {
			if (
				this._findState.query !== state.query ||
	...
	a11yWillPlaceVirtCursorOnSearchResult = debounce(async () => {
		...
		let page =
			this._iframeWindow.PDFViewerApplication.pdfViewer.getPageView(
				position.pageIndex
			);
		...
		let node = endNode.textContent.includes(this._findState.query)
			? endNode
			: startNode;
	...
	_setSelectionRanges(selectionRanges) {
		...
		if (selectionRange && !selectionRange.collapsed) {
			...
			let annotation = this._getAnnotationFromSelectionRanges(
				this._selectionRanges,
				"highlight"
			);
			if (!this._isTranslationView) {
	...
	setZoomState(zoomState) {
		...
		if (
			typeof zoomState.scale === "number" &&
			!Number.isNaN(zoomState.scale)
		) {
	...
	async _pushHistoryPoint() {
		...
		let { pageNumber, top, left } =
			this._iframeWindow.PDFViewerApplication.pdfViewer._location;
		let pageIndex = pageNumber - 1;
		this._history.save({
			dest: [pageIndex, { name: "XYZ" }, left, top, null],
		});
	}
	...
	async navigate(location, options = {}) {
		...
		if (
			...
			let annotation = this._annotations.find(
				(x) => x.id === location.annotationID
			);
			this.navigateToPosition(annotation.position, options);
		} else if (location.dest) {
			this._iframeWindow.PDFViewerApplication.pdfLinkService.goToDestination(
				location.dest
			);
			...
			let pageIndex = this._pageLabels.findIndex(
				(x) => x === location.pageLabel
			);
			if (pageIndex !== -1) {
				this._iframeWindow.PDFViewerApplication.pdfViewer.scrollPageIntoView(
	...
	getViewPoint(point, pageIndex, tm = [1, 0, 0, 1, 0, 0]) {
		let originalPage =
			this._iframeWindow.PDFViewerApplication.pdfViewer._pages[pageIndex];
		let scaleTransform = [
			originalPage.outputScale.sx,
			0,
			0,
			originalPage.outputScale.sy,
			0,
			0,
		];
	...
	getClientRectForPopup(position, scrollLeft, scrollTop) {
		let pageIndex = position.nextPageRects
		...
		let [x2, y1] = page.viewport.convertToViewportPoint(
			...rect.slice(2, 4)
		);
		...
		if (!this._pp || 1) {
			...
			return [pr.x + r[0], pr.y + r[1], pr.x + r[2], pr.y + r[3]];
		} else {
			let pp = this._pp;
			let x = pp.pr.x;
			let y = pp.pr.y;
			y += pp.scrollTop - scrollTop;
			return [pp.pr.x + r[0], y + r[1], pp.pr.x + r[2], y + r[3]];
		}
	}
	...
	getSelectedAnnotationAction(annotation, position) {
		...
		if (
			...
			(annotation.position.nextPageRects &&
				!this._pdfPages[annotation.position.pageIndex + 1])
		) {
			return null;
		}
		...
		if (
			!this._iframeWindow.PDFViewerApplication.pdfViewer._pages[
				position.pageIndex
			].outputScale
		) {
			return null;
		...
		if (["highlight", "underline"].includes(annotation.type)) {
			...
			if (
				this._pdfPages[annotation.position.pageIndex] &&
				(!annotation.position.nextPageRects ||
					this._pdfPages[annotation.position.pageIndex + 1])
			) {
				...
				if (annotation.position.nextPageRects) {
					if (
						...
						let { chars } =
							this._pdfPages[annotation.position.pageIndex + 1];
						let rotation = getRectRotationOnText(
							chars,
							annotation.position.nextPageRects.at(-1)
						);
						// Add page rotation to text rotation
						rotation += this.getViewportRotation(
							annotation.position.pageIndex + 1
						);
					...
					rotation = getRectRotationOnText(
						chars,
						annotation.position.rects.at(-1)
					);
					// Add page rotation to text rotation
					rotation += this.getViewportRotation(
						annotation.position.pageIndex
				...
				if (startHandle) {
					...
					if (quickIntersectRect(rect, p)) {
						...
						selectionRanges =
							getReversedSelectionRanges(selectionRanges);
						return {
			...
			let bottomLeft = this.getViewPoint(
				[r[0], r[1]],
				position.pageIndex,
				tm
			);
			let bottomRight = this.getViewPoint(
				[r[2], r[1]],
				position.pageIndex,
				tm
			);
			let topLeft = this.getViewPoint(
				[r[0], r[3]],
				position.pageIndex,
				tm
			);
			let topRight = this.getViewPoint(
				[r[2], r[3]],
				position.pageIndex,
				tm
			);
			let middleLeft = this.getViewPoint(
				[r[0], r[1] + (r[3] - r[1]) / 2],
				position.pageIndex,
				tm
			);
			let middleRight = this.getViewPoint(
				[r[2], r[1] + (r[3] - r[1]) / 2],
				position.pageIndex,
				tm
			);
			let middleTop = this.getViewPoint(
				[r[0] + (r[2] - r[0]) / 2, r[3]],
				position.pageIndex,
				tm
			);
			let middleBottom = this.getViewPoint(
				[r[0] + (r[2] - r[0]) / 2, r[1]],
				position.pageIndex,
				tm
			);
			let ROTATION_BOTTOM = 16;
			let rotation = this.getViewPoint(
				[r[0] + (r[2] - r[0]) / 2, r[3] + ROTATION_BOTTOM],
				position.pageIndex,
				tm
			);
			...
			if (
				...
				quickIntersectRect(rotationRect, p)
			) {
				...
				["image", "text"].includes(annotation.type) &&
				quickIntersectRect(middleLeftRect, p)
			) {
				dir = "l";
			} else if (
				["image", "text"].includes(annotation.type) &&
				quickIntersectRect(middleRightRect, p)
			) {
				dir = "r";
			} else if (
				annotation.type === "image" &&
				quickIntersectRect(middleTopRect, p)
			) {
				dir = "t";
			} else if (
				annotation.type === "image" &&
				quickIntersectRect(middleBottomRect, p)
			) {
				dir = "b";
			}
			if (dir) {
			...
			if (inside && !this._textAnnotationFocused()) {
				...
				let br = getPositionBoundingRect(annotation.position);
				return {
					type: ["text", "ink"].includes(annotation.type)
						? "moveAndDrag"
						: "drag",
					annotation,
					x: r[0] - br[0],
		...
		if (
			...
			let br = getPositionBoundingRect(annotation.position);
			return {
				type: ["note", "text", "ink"].includes(annotation.type)
					? "moveAndDrag"
					: "drag",
				annotation,
				x: r[0] - br[0],
	...
	_getPageAnnotations(pageIndex) {
		return this._annotations.filter(
			(x) =>
				x.position.pageIndex === pageIndex ||
				(x.position.nextPageRects &&
					x.position.pageIndex + 1 === pageIndex)
		);
	}
	...
	getSelectableAnnotations(position) {
		...
		for (let annotation of annotations) {
			if (annotation.type === "text" && annotation.position.rotation) {
				let tm = getRotationTransform(
					annotation.position.rects[0],
					annotation.position.rotation
				);
		...
		let selectedTextAnnotation = selectableAnnotations.find(
			(x) => x.type === "text" && x.id === this._selectedAnnotationIDs[0]
		);
	...
	getSelectedAnnotations() {
		return this._annotations.filter((x) =>
			this._selectedAnnotationIDs.includes(x.id)
		);
	}
	...
	getActionAtPosition(position, event) {
		...
		if (this._tool.type === "ink") {
			...
			return {
				action: { type: "erase", annotations: new Map() },
				selectAnnotations: [],
			};
		}
		...
		let selectableAnnotation = (this.getSelectableAnnotations(position) ||
			[])[0];
		...
		if (selectableAnnotation) {
			...
			// If annotation isn't selected
				...
				if (
					!(this._readOnly || annotation.readOnly) &&
					["note", "text", "ink"].includes(annotation.type)
				) {
					let r = position.rects[0];
					let br = getPositionBoundingRect(annotation.position);
					action = {
			...
			if (this._tool.type === "note") {
				...
				if (
					...
					["highlight", "underline"].includes(this._tool.type)
				) {
	...
	updateCursor(action) {
		...
		if (action) {
			if (action.type === "overlay") {
				...
				let transform =
					this._iframeWindow.PDFViewerApplication.pdfViewer._pages[0]
						.viewport.transform;
				let [a, b] = transform;
				...
				let svgDataUrl = [
					...
					`            cy="${adjustedSize / 2}"`,
					`            r="${
						(adjustedSize - adjustedStrokeWidth) / 2
					}"`,
					`            stroke="${color}"`,
	...
	_handlePointerDown(event) {
		...
		let page = this.getPageByIndex(position.pageIndex);
		let { action, selectAnnotations } = this.getActionAtPosition(
			position,
			event
		);
		...
		if (
			selectAnnotations &&
			!(
				selectAnnotations.length === 0 &&
				this._selectedAnnotationIDs.length === 0
			)
		) {
			this._onSelectAnnotations(
				selectAnnotations.map((x) => x.id),
				event
			);
		...
		if (action.type === "note") {
			...
			this._onAddAnnotation(
				{
					...
					pageLabel: this._getPageLabel(
						this.pointerDownPosition.pageIndex,
						true
					),
			...
			let updatedAnnotations = eraseInk(
				x,
				y,
				this._tool.size,
				annotations
		...
		if (action.type === "selectText") {
			if (event.detail === 1 || !event.detail) {
				if (shift && this._selectionRanges.length) {
					this._selectionRanges = getModifiedSelectionRanges(
						this._pdfPages,
						this._selectionRanges,
						position
					);
				} else {
					this._selectionRanges = getSelectionRanges(
						this._pdfPages,
						position,
						position
					);
				...
				this._selectionRanges = getWordSelectionRanges(
					this._pdfPages,
					position,
					position
				);
				...
				this._selectionRanges = getLineSelectionRanges(
					this._pdfPages,
					position,
					position
				);
	...
	_handlePointerMove = throttle(
		(event) => {
			...
			if (!this.pointerDownPosition) {
				...
				let position = this.pointerEventToPosition(event);
				if (position) {
					let { action, selectAnnotations } =
						this.getActionAtPosition(position, event);
					...
					if (overlay) {
						if (
							["citation", "reference"].includes(overlay.type) ||
						...
						for (let page of this._iframeWindow.PDFViewerApplication
							.pdfViewer._pages) {
							if (page.div.title) {
								page.div.title = "";
							}
								...
								async () => {
									...
									if (overlayPopup.type === "internal-link") {
										let { image, width, height, x, y } =
											await this._pdfRenderer?.renderPreviewPage(
			...
			let originalPagePosition = this.pointerEventToAltPosition(
				event,
				this.pointerDownPosition.pageIndex
			);
			let position = this.pointerEventToPosition(event);
			...
			if (action.type === "updateAnnotationRange") {
				action.selectionRanges = getModifiedSelectionRanges(
					this._pdfPages,
					action.selectionRanges,
					position
				);
				let {
					sortIndex,
				...
				if (action.annotation.type === "ink") {
					...
					let rect = getPositionBoundingRect(
					...
					let r1 = getPositionBoundingRect(
						action.annotation.position
					);
					...
					action.position = applyTransformationMatrixToInkPosition(
						mm,
						action.annotation.position
					);
					action.triggered = true;
					...
					if (action.dir.includes("l")) {
						x =
							(x > rect[2] - MIN_IMAGE_ANNOTATION_SIZE &&
								rect[2] - MIN_IMAGE_ANNOTATION_SIZE) ||
							(x > viewBox[0] && x) ||
							viewBox[0];
					} else if (action.dir.includes("r")) {
						x =
							(x < rect[0] + MIN_IMAGE_ANNOTATION_SIZE &&
								rect[0] + MIN_IMAGE_ANNOTATION_SIZE) ||
							(x < viewBox[2] && x) ||
							viewBox[2];
					}
					if (action.dir.includes("b")) {
						y =
							(y > rect[3] - MIN_IMAGE_ANNOTATION_SIZE &&
								rect[3] - MIN_IMAGE_ANNOTATION_SIZE) ||
							(y > viewBox[1] && y) ||
							viewBox[1];
					} else if (action.dir.includes("t")) {
						y =
							(y < rect[1] + MIN_IMAGE_ANNOTATION_SIZE &&
								rect[1] + MIN_IMAGE_ANNOTATION_SIZE) ||
							(y < viewBox[3] && y) ||
							viewBox[3];
					}
					...
					action.position = JSON.parse(
						JSON.stringify(action.annotation.position)
					);
					...
					if (action.annotation.position.rotation) {
						let tm = getRotationTransform(
							rect,
							action.annotation.position.rotation
						);
					...
					let scale = calculateScale(
						action.annotation.position.rects[0],
						rect
					);
					if (action.dir.length !== 2) {
						scale = 1;
			...
			// Only note and image annotations are supported
				...
				const PADDING = 5;
				x =
					(x > viewBox[2] - PADDING && viewBox[2] - PADDING) ||
					(x < viewBox[0] + PADDING && viewBox[0] + PADDING) ||
					x;
				y =
					(y > viewBox[3] - PADDING && viewBox[3] - PADDING) ||
					(y < viewBox[1] + PADDING && viewBox[1] + PADDING) ||
					y;
				...
				if (action.annotation.type === "ink") {
					...
					let position2 = applyTransformationMatrixToInkPosition(
						mm,
						action.annotation.position
					);
				...
				let [left, bottom, right, top] =
					page.originalPage.viewport.viewBox;
				action.annotation = {
					type: "image",
					color: this._tool.color,
					pageLabel: this._getPageLabel(
	...
	_getAnnotationFromSelectionRanges(selectionRanges, type, color) {
		...
		let annotation = {
			...
			pageLabel: this._getPageLabel(
				selectionRange.position.pageIndex,
				true
			),
	...
	_handlePointerUp(event) {
		...
		if (
			!this.action &&
			event.target.classList?.contains("textAnnotation")
		) {
			return;
		}
		...
		if (this.pointerDownPosition) {
			...
			if (action) {
				if (action.triggered) {
					if (action.type === "updateAnnotationRange") {
						action.annotation.sortIndex = getSortIndex(
							this._pdfPages,
							action.annotation.position
						);
						...
						if (
							width >= MIN_IMAGE_ANNOTATION_SIZE &&
							height >= MIN_IMAGE_ANNOTATION_SIZE
						) {
							action.annotation.sortIndex = getSortIndex(
								this._pdfPages,
								action.annotation.position
						...
						let lastInkAnnotation = this._annotations.find(
							(x) => x.id === this._lastAddedInkAnnotationID
						);
						...
						if (lastInkAnnotation) {
							let r1 = getPositionBoundingRect(
								lastInkAnnotation.position
							);
							let r2 = getPositionBoundingRect(
								action.annotation.position
							);
						...
						if (
							lastInkAnnotation &&
							lastInkAnnotation.position.pageIndex ===
								action.annotation.position.pageIndex &&
							lastInkAnnotation.position.width ===
								action.annotation.position.width &&
							lastInkAnnotation.color ===
								action.annotation.color &&
							Date.now() -
								Date.parse(lastInkAnnotation.dateModified) <
								10 * 1000 &&
							dist < 50
						) {
							let { id, position } = lastInkAnnotation;
							let paths =
								lastInkAnnotation.position.paths.slice();
						...
						let updated = annotations.filter(
							(x) => x.position.paths.length
						);
						let deleted = annotations.filter(
							(x) => !x.position.paths.length
						);
						if (updated.length) {
					...
					position &&
					!this.action.alreadySelectedAnnotations &&
					this._tool.type !== "eraser"
				) {
					let selectableAnnotations =
						this.getSelectableAnnotations(position);
					let lastSelectedAnnotationID =
						this._selectedAnnotationIDs.slice(-1)[0];
					let annotation = selectableAnnotations.find(
						(annotation) =>
							annotation.id === lastSelectedAnnotationID
					);
					if (annotation?.type === "text") {
						let node = this._iframeWindow.document.querySelector(
					...
					let indexOfCurrentID =
						selectableAnnotations.indexOf(annotation);
					if (indexOfCurrentID !== -1) {
						if (
							indexOfCurrentID <
							selectableAnnotations.length - 1
						) {
							nextID =
								selectableAnnotations[indexOfCurrentID + 1].id;
				...
				if (action.type === "selectText") {
					...
					if (["highlight", "underline"].includes(this._tool.type)) {
						if (
							...
							annotation.sortIndex = getSortIndex(
								this._pdfPages,
								annotation.position
							);
						...
						if (selectionRange && !selectionRange.collapsed) {
							...
							annotation.pageLabel = this._getPageLabel(
								annotation.position.pageIndex,
								true
							);
							this._onSetSelectionPopup({ rect, annotation });
	...
	_handleContextMenu(event) {
		...
		if (this._options.platform !== "web" && event.button === 2) {
			...
			if (position) {
				selectableAnnotation = (this.getSelectableAnnotations(
					position
				) || [])[0];
			}
			let selectedAnnotations = this.getSelectedAnnotations();
			if (!selectableAnnotation) {
				...
				if (
					...
					let selectionBoundingRect = this.getClientRectForPopup(
						this._selectionRanges[0].position
					);
					this._onOpenViewContextMenu({
						x: br.x + selectionBoundingRect[0],
						y:
							br.y +
							selectionBoundingRect[3] +
				...
				!selectedAnnotations.includes(selectableAnnotation) &&
				!this._textAnnotationFocused()
			) {
				this._onSelectAnnotations([selectableAnnotation.id], event);
				this._onOpenAnnotationContextMenu({
					ids: [selectableAnnotation.id],
					x: br.x + event.clientX,
					y: br.y + event.clientY,
					view: true,
				});
				...
				this._onOpenAnnotationContextMenu({
					ids: selectedAnnotations.map((x) => x.id),
					x: br.x + event.clientX,
					y: br.y + event.clientY,
					view: true,
				});
	...
	_handleKeyDown(event) {
		...
		// This is necessary when a page is zoomed in and left/right arrow keys can't change page
			...
			!this._annotations.find(
				(x) => x.id === this._selectedAnnotationIDs[0]
			)?.readOnly
		) {
			let annotation = this._annotations.find(
				(x) => x.id === this._selectedAnnotationIDs[0]
			...
			if (
				...
				if (dx || dy) {
					...
					if (annotation.type === "ink") {
						let m = [1, 0, 0, 1, dx, dy];
						position = applyTransformationMatrixToInkPosition(
							m,
							position
						);
					} else {
				...
				["highlight", "underline"].includes(type) &&
				[
					"Shift-ArrowLeft",
					"Shift-ArrowRight",
					"Shift-ArrowUp",
				...
				if (key === "Shift-ArrowLeft") {
					selectionRanges = getModifiedSelectionRanges(
						this._pdfPages,
						selectionRanges,
						"left"
					);
				...
				if (
					!(
						...
						selectionRanges[0].anchorOffset >=
							selectionRanges[0].headOffset
					)
				) {
					let annotation2 = this._getAnnotationFromSelectionRanges(
						selectionRanges,
						"highlight"
					);
					let { text, sortIndex, position } = annotation2;
					this._onUpdateAnnotations([
						{ id, text, sortIndex, position },
					]);
					this._onSetAnnotationPopup();
				}
				event.stopPropagation();
				...
				if (type === "ink") {
					...
					if (key === "Shift-ArrowLeft") {
						let x = rect[2] - STEP;
						rect[2] =
							(x < rect[0] + MIN_TEXT_ANNOTATION_WIDTH &&
								rect[0] + MIN_TEXT_ANNOTATION_WIDTH) ||
							x;
						modified = true;
						dir = "r";
					} else if (key === "Shift-ArrowRight") {
			...
			if (annotation) {
				...
				setTimeout(() => {
					this._iframeWindow.document
						.querySelector(`[data-id="${annotation.id}"]`)
						?.focus();
				}, 100);
			}
	...
	_handleDragStart(event) {
		...
		if (
			!this.action ||
			!["moveAndDrag", "drag"].includes(this.action.type)
		) {
			event.preventDefault();
			return;
		}
		if (!this.action.multiple) {
			...
			if (annotation.type === "text") {
				...
				let pixelRatio =
					isSafari || isFirefox || 1 ? window.devicePixelRatio : 1;
				...
				event.dataTransfer.setDragImage(
					canvas,
					this.action.x * scale,
					canvas.height / pixelRatio - this.action.y * scale
				);
			}
		}
	...
	_handleCopy(event) {
		...
		// Copying text
			...
			if (this._selectionRanges.length <= 2) {
				...
				let fullText = getTextFromSelectionRanges(
					this._selectionRanges
				);
	...
	pointerEventToPosition(event) {
		let target = this._iframeWindow.document.elementFromPoint(
			event.clientX,
			event.clientY
		);
		if (!target) {
		...
		let pageIndex =
			this._iframeWindow.PDFViewerApplication.pdfViewer._pages.findIndex(
				(x) => x.div === div
			);
		if (pageIndex < 0) {
			return null;
	...
	_textAnnotationFocused() {
		return this._iframeWindow.document.activeElement.classList.contains(
			"textAnnotation"
		);
	}

	setScrollMode(mode) {
		this._iframeWindow.PDFViewerApplication.eventBus.dispatch(
			"switchscrollmode",
			{ mode }
		);
	}
	...
	async setSidebarView(sidebarView) {
		...
		if (sidebarView === "outline" && !this._outline) {
			...
			let outline =
				await this._iframeWindow.PDFViewerApplication.pdfDocument.getOutline2();
			this._onSetOutline(outline);
		}
	}
	...
	async _getPositionFromDestination(dest) {
		...
		if (typeof dest === "string") {
			...
			if (!destArray) {
				throw new Error(
					`Unable to resolve named destination: "${dest}"`
				);
			}
		} else {
			destArray = dest;
